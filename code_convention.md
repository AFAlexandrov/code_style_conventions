...
Под блоком понимаем две пары файлов .c и .h
С явно разделёнными открытым и закрытым интерфейсами
При этом объявления строго в заголовках (иногда кроме inline)
А реализации строго в .c

TODO
1. варианты связи блоков
1.1. через открытые интерфейсыы
1.2. через закрытые
1.3. кольцом
...
// Старайся не перегружать текстом, формулируй как можно короче
Вокруг бинарных ставятся пробелы <--------------------------\
                                                             |
// это не лучший вариант, но он не заставляет вчитываться __/
...
Те же принципы для унарных и бинарных операторов (это наверное лучше не писать)

		uint8_t zeros_in_mask &= ~msk;

*Составное присваивание* !!! это видимо то, что выше написал..)
