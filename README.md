...
* Один таб - 4 пробела.
* Метки case выравнивать по ключевому слову switch.
* Не располагать несколько выражений на одной строке, кроме ситуаций, когда необходимо что-то спрятать.
* Не допускать множественного присваивания на одной строке.
...
Таким образом, поскольку символ * чаще относится к идентификатору, следует отделять его от типа, но не от идентификатора.

*Несколько идентификаторов рекомендуется писать через запятую с переносом строки и одним отступом везде, 
где это не противоречит логической связности перечисляемых идентификаторов*
...
* Однако, нет необходимости начинать с подчёркивания функции, находящиеся в файлах \*\_internal.{c,h}
...
Для тернарного оператора при использовании " : " ставятся пробелы по обе стороны, а последний операнд записывается на новой строке
		...
		// Первый случай стоит использовать, если первый аргумент функции - дескриптор сущности или экземпляр "класса".
